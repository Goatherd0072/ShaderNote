Shader "Chapter10/Reflect_EmpricialFresnel"
{
    // Empricial 菲涅尔反射 Fresnel
    Properties
    {
        ...
        _Bias("Fresnel Bias", float) = 0.1
        _Scale("Fresnel Scale", float) = 0.5
        _Power("Fresnel Power", float) = 5.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Varyings i) : SV_Target
            {
                ...
                float3 reflectMap = SAMPLE_TEXTURECUBE(_reflectMap, sampler_reflectMap, i.worldReflect).xyz;
                ...
                // Empricial 菲涅尔反射 Fresnel
                float fresnel = max(0.0f ,( min(1.0f,( _Bias  + _Scale * pow(1.0f-(i.worldView * i.normalWS), _Power)))));

                float3 output = Ambient + lerp(Diffuse, reflectMap, fresnel) * mainLight.distanceAttenuation;
