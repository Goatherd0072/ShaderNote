Shader "Chapter8/AlphaBlend_CullOff"
{
    // Alpha混合双面剔除，因为ZWrite被关闭，为了确保透明物体的正面和背面都能正确显示，
    Properties
    {
    ...
    SubShader
    {
        ...
        // Build-in shader可能需要两个PASS来做剔除（一个Pass剔除正面，一个Pass剔除背面），来得到目标效果
        // 但是 URP 可以直接在一个Pass中使用Cull Off来实现，暂时不知道原因
        Pass
        {
            Name "AlphaBlend_CullOff"
            Tags
            {
            ...
            // 其他 https://docs.unity.cn/cn/2020.3/Manual/SL-Blend.html
            ...
            float4 frag (Varyings i) : SV_Target
            {
                ...
                float3 Specular = _Specular * lightCol * pow(saturate(dot( normal, halfDir)), gloss) ;// 高光反射

                return float4( Ambient + Diffuse + Specular, _AlphaScale * baseMap.a);
            }
            ENDHLSL
        }
    }
